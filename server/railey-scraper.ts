...
export class RaileyScraper {
  ...
  async fetchListings(): Promise<RaileyProperty[]> {
    ...
    if (this.lastFetched && Date.now() - this.lastFetched.getTime() < this.cacheTimeout) {
  ...
  private parseListingsFromHtml(html: string): RaileyProperty[] {
    ...
    const listingPattern = /MLS#:\s*(\d+).*?href="([^"]*)".*?\$([0-9,]+).*?([^<]*)<.*?(\d+)\s+Bed.*?(\d+(?:\.\d+)?)\s+Bath.*?(\d+,?\d*)\s+SqFt/g;
    
    let match;
    while ((match = listingPattern.exec(html)) !== null && properties.length < 50) {
      const [, mlsNumber, url, priceStr, addressLine, beds, baths, sqft] = match;
      ...
      const imagePattern = new RegExp(`img[^>]*src="([^"]*)"[^>]*alt="[^"]*${mlsNumber}[^"]*"`);
      const imageMatch = html.match(imagePattern);
      const imageUrl = imageMatch ? imageMatch[1] : 'https://images.unsplash.com/photo-1570129477492-45c003edd2be?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&h=300';
  ...
  private extractFeatures(address: string, sqft: number, price: number): string[] {
    const features: string[] = [];
    
    // Location-based features
    if (address.toLowerCase().includes('deep creek') || address.toLowerCase().includes('lake')) {
      features.push('Lake Access', 'Mountain Views');
    }
    if (address.toLowerCase().includes('wisp') || address.toLowerCase().includes('ski')) {
    ...
    if (features.length === 0) {
      features.push('Mountain Living', 'Natural Setting', 'Four Season Location');
    }
  ...
  private generateDescription(address: string, features: string[], price: number): string {
    const location = address.includes('MD') ? 'Maryland mountain' : 'scenic';
    const priceRange = price > 500000 ? 'luxury' : price > 300000 ? 'premium' : 'comfortable';
    ...
    return [
      {
        ...
        description: 'Stunning lakefront property with private dock access and panoramic mountain views. Perfect for year-round living or vacation rental.',
      ...
      },
      {
        ...
        description: 'Modern condo with direct ski slope access at Wisp Resort. Fully furnished and ready for mountain adventures.',
        features: ['Ski Access', 'Resort Amenities', 'Furnished', 'Mountain Views'],
        daysOnMarket: 8,
        mlsNumber: 'SAMPLE002'
      },
      {
        ...
        description: 'Spacious family home with breathtaking mountain views and large yard. Great for full-time residence in beautiful Garrett County.',
        features: ['Mountain Views', 'Large Yard', 'Family Friendly', 'Updated Kitchen'],
        daysOnMarket: 22,
        mlsNumber: 'SAMPLE003'
      },
      {
        ...
        bathrooms: '1',
        sqft: 1000,
        imageUrl: 'https://images.unsplash.com/photo-1518780664697-55e3ad937233?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&h=300',
        description: 'Cozy cabin with lake access and hiking trails nearby. Perfect weekend getaway in the heart of nature.',
        features: ['Lake Access', 'Hiking Trails', 'Cozy Cabin', 'Natural Setting'],
        daysOnMarket: 5,
        mlsNumber: 'SAMPLE004'
      },
      {
        ...
        description: 'Impressive luxury estate with premium finishes, multiple fireplaces, and expansive mountain views from every room.',
        features: ['Luxury Estate', 'Premium Finishes', 'Multiple Fireplaces', 'Panoramic Views'],
        daysOnMarket: 45,
        mlsNumber: 'SAMPLE005'
      }
    ];
